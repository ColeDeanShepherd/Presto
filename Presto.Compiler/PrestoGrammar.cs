...
public static class PrestoGrammarConstants
{
    public static List<(Regex Regex, TokenType TokenType)> LexerGrammar = new()
    {
        ...
        (new Regex(@"\["), TokenType.LeftSquareBracket),
        (new Regex(@"\]"), TokenType.RightSquareBracket),
    ...
    public static readonly List<GrammarRule> Grammar = new()
    {
        ...
        Rule(
            ...
            Token(TokenType.LeftParen), TokenSeparated(RuleRef("ParameterDefinition"), TokenType.Comma), Token(TokenType.RightParen),
            Token(TokenType.LeftCurlyBracket), RuleRef("Statement").ZeroOrMore(), Token(TokenType.RightCurlyBracket)),
        Rule(
            "StructDeclaration",
        ...
        Rule(
            ...
            Optional(Group(Token(TokenType.LeftSquareBracket), TokenSeparated(RuleRef("Attribute"), TokenType.Comma, OneOrMore: true), Token(TokenType.RightSquareBracket)))),
        Rule(
            "Attribute",
            children => new AttributeReference(children),
            RuleRef("QualifiedName")),
        Rule(
            "FieldDeclaration",
            children => new FieldDeclaration(children),
            Token(TokenType.Identifier), Token(TokenType.Colon), RuleRef("QualifiedName")),
